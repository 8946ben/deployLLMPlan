...
| ④ 硬件库 | 按品牌/代际分组陈列、代际内按性能从强到弱排序:19 款 GPU 卡(NVIDIA 的 V/T/A/40/H/B 系列与 AMD MI)、5 款 CPU 平台(Intel/AMD)、DDR5 内存条、**FPGA**(Alinx AXU9EGB,规格取自 LLM_FPGA 实测工程)、**整机 4 成员**(DGX-1、思腾合力、Mac Studio M2 Ultra、DGX Spark——添加时自动展开为部件),点击或拖放到画板 |
