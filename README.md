...
| ③ 服务场景 | 服务上下文长度、并发路数、提示词长度(TTFT 估算用) |
| ④ 硬件库 | 15 款 GPU(V100/T4/3090/4090/5090/A10/L40S/A100/H100/MI250X/M2 Ultra)、3 款 CPU 平台、DDR5 内存条,点击或拖放到画板 |
| ⑤ 部署画板 | 多卡/多内存自由组合,实时汇总显存、带宽、内存 |
| 📊 分析报告 | ✅/⚠️/❌ 部署判定 + 原因;显存构成条形图(权重/KV/开销/缺口);架构兼容性检查清单;解码/预填充速度、TTFT、最大并发、总吞吐、瓶颈分析;可执行的改进建议 |

右上角「快速场景」内置 6 个示例:V100 跑 32B 失败分析、llama.cpp Q4 成功部署、CPU 卸载混合推理、4×H100 FP8 集群、MoE + Apple 统一内存、纯 CPU 部署。
...
- **解码速度**(带宽受限):`tok/s ≈ 有效带宽 / 每 token 读取字节(激活权重 + 全量KV)`
  - 张量并行(TP):各卡并行读分片,取最慢卡;无 NVLink 时 ×0.8 通信惩罚
  - 层切分(llama.cpp 多卡/卸载):各卡串行累加 → 多卡主要扩容不明显提速
  - 框架效率系数:vLLM 0.75 / TRT-LLM 0.85 / llama.cpp 0.70 / Transformers 0.50
...
- 多卡:TP 混插不同型号告警;无 NVLink 的 TP 告警;TP 均分超过最小卡显存告警
